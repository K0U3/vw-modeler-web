...
def explain_error(e):
    ...
    if '躯体ポリライン' in str(e):
        return ('壁のレイヤーが認識できない（レイヤー名が「躯体・壁・建具・'
                '外壁・内壁」等の日本語標準名と異なるか、壁が閉ポリライン'
                'で描かれていない）',
                'DXFのレイヤー名に「壁」「躯体」等を含めるか、'
                '図面を相川まで送ってください')
...
def build_script(dxf_path, overrides=None):
    ...
    a("    num_digits(no, (x1 + x2) / 2, (y1 + y2) / 2)   # 立体の番号（3Dビュー用）")
    a('')
    ...
    a('RED = (65535, 0, 0)')
    a('')
    a("_SEG7 = {   # 7セグメント表示の点灯パターン")
    a("    '0': 'ABCDEF', '1': 'BC', '2': 'ABGED', '3': 'ABGCD', '4': 'FGBC',")
    a("    '5': 'AFGCD', '6': 'AFGECD', '7': 'ABC', '8': 'ABCDEFG', '9': 'ABCDFG',")
    a('}')
    a('')
    a('def _seg_rect(x1, y1, x2, y2):')
    a('    """番号セグメント: 天井の上に浮かせた赤い薄板（3Dビューで必ず見える）"""')
    a('    vs.BeginXtrd(0, 30)')
    a('    vs.Rect(x1 + OX, y1 + OY, x2 + OX, y2 + OY)')
    a('    vs.EndXtrd()')
    a('    obj = vs.LNewObj()')
    a('    vs.Move3DObj(obj, 0, 0, CH + 300)')
    a('    try:')
    a('        vs.SetFPat(obj, 1)')
    a('        vs.SetFillFore(obj, RED)')
    a('        vs.SetFillBack(obj, RED)')
    a('        vs.SetPenFore(obj, RED)')
    a('    except Exception:')
    a('        pass')
    a('')
    a('def num_digits(no, x, y):')
    a('    """窓番号を7セグ形状の立体で窓の真上に表記する。')
    a('    テキストはVWの3Dビューに表示されないため、ジオメトリで描く"""')
    a('    if not SHOW_NUMBERS:')
    a('        return')
    a('    s = str(no)')
    a('    W, H, T, GAP = 260.0, 440.0, 60.0, 90.0')
    a('    x0 = x - (len(s) * (W + GAP) - GAP) / 2.0')
    a('    y0 = y - H / 2.0')
    a('    for ch in s:')
    a("        segs = _SEG7.get(ch, '')")
    a("        if 'A' in segs:")
    a('            _seg_rect(x0, y0 + H - T, x0 + W, y0 + H)')
    a("        if 'D' in segs:")
    a('            _seg_rect(x0, y0, x0 + W, y0 + T)')
    a("        if 'G' in segs:")
    a('            _seg_rect(x0, y0 + (H - T) / 2, x0 + W, y0 + (H + T) / 2)')
    a("        if 'F' in segs:")
    a('            _seg_rect(x0, y0 + H / 2, x0 + T, y0 + H)')
    a("        if 'E' in segs:")
    a('            _seg_rect(x0, y0, x0 + T, y0 + H / 2)')
    a("        if 'B' in segs:")
    a('            _seg_rect(x0 + W - T, y0 + H / 2, x0 + W, y0 + H)')
    a("        if 'C' in segs:")
    a('            _seg_rect(x0 + W - T, y0, x0 + W, y0 + H / 2)')
    a('        x0 += W + GAP')
    a('')
    a('def _door_h(no):')
